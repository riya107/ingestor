    ...
    for resume in resume_names:
        ...
        if resume.endswith('pdf'):
            loader = PyPDFLoader(os.path.join(
                config.RESUMES_PATH, resume_path, resume))
            documents.extend(
        ...
        # Load and split TXT documents
            ...
            documents.extend(config.TEXT_SPLITTER.create_documents([text_file], metadatas=[
                             {'source': os.path.join(config.RESUMES_PATH, resume_path, resume)}]))
    # Create and persist the vector database
    vectordb = Chroma.from_documents(
        documents=documents, persist_directory=config.VECTORDB_PATH)
    # Ensuring that the vector database is persisted
    vectordb.persist()
